  ...
  <!--Header-->
	...
	<form name="formulario" action="ListaDestacados.jsp" class="form-horizontal mitad"
		...
		<fieldset>
			...
			<legend> Contenido de cursos destacados </legend>
			...
			<!-- You can make it whatever width you want. I'm making it full width
             on <= small devices and 4/12 page width on >= medium devices -->
				<div class="col-xs-14 col-md-6">
					<h4>Informaci&oacute;n sobre los cursos destacados</h4>
					<% 
					...
					for(Curso curso: Listacursos) {
						%>
					<!-- 	<li><%//=curso.getTitulo() %> _____ <%//curso.getDestacado()%> <br>  -->
						<tr> <!-- &nbsp; esto sirve para meter un espacio  tambien aprovecho para meter la minicopmaracion para mostrar si esta validado o no-->
								<td><%=curso.getTitulo() %></td>   <td><%if(curso.getDestacado()== false){%>Curso no destacado <%}else{%> Curso Destacado <%}%></td> <td><%//=curso.getImagenuri() %></td>
						</tr>
						<br>
						Estado de LOS CURSOS DESTACADOS
						
						
						 <br>
						<%String valor=curso.getTitulo(); %>
						<%if(curso.getDestacado()==false){// si esto no se cumple es que es verdadderos entonces no necesatias destacarlo otra vez %>
						<br/> <br/> ¿Deseas Destacar este curso?:<br/> <!-- paso el titulo de los cursos que voy a destacar -->
						<input class="messageCheckbox" type="checkbox" name="entornosDestacados" value= <%=valor%> ><label for='chk_email_alerts'>Destacar curso </label> <br/>
												
						<%} // fin del if de destacar cursos igual a false, 
						else if (curso.getDestacado()==true){%>
							<p>Este curso ya esta destacado</p>
					...
					<input type="submit" class="btn btn-primary btn-lg btn-block" value ="Enviar">Aceptar</button>
					
					<br>
					<br>
					<% Curso curso= new Curso(); %>
					<a href="ServletCursos" > Ir al listado de cursos </a></li><br>
