...
<head>
  ...
  <title>Lista de destacados</title>
...
<body>
  <!--Header-->
	...
	<form action="ServletLecciones" enctype="multipart/form-data"
		...
		<hr>
		<fieldset>
			<h1>Estas en la lista de destacados</h1>
	
	<ul>
	<%
	String []seleccionados = request.getParameterValues("entornosDestacados");
	if(seleccionados!=null){//si no hay nada no puedes recorrer asi q no entras
		for(int i=0; i<seleccionados.length;i++ ){// recorro solo aquellos q valido
			%>
			<li><!-- esto es opcional este print se puede quitar lo dejo solo para controlar son los cursos que han sido marcados en el destacar.jsp-->
			<%=seleccionados[i] %>
			</li>
	...
	if(seleccionados!=null){// la lista de seleccionados 
		for(Curso curso: Listacursos) {// recorro la lista de cursos
			...
			for (int k=0; k<seleccionados.length; k++){
				if (curso.getTitulo().equalsIgnoreCase(seleccionados[k])){// si los titulos conciden es xq es verdadero
					curso.setDestacado(true);//digo q es verdadero
				}
		...
		for(Curso curso: Listacursos){// recorro solo aquellos q destaco
			%>
	...
	%>
	<a href="Destacar.jsp" >Volver a la lista de cursos destacados  </a>
	</ul>
